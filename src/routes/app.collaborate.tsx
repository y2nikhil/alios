...
type Channel = { id: string; team_id: string | null; name: string };
type Msg = {
  id: string;
  channel_id: string;
  user_id: string;
...
function CollaboratePage() {
  ...
  // Load global channels + team channels
  useEffect(() => {
    if (!user) return;
    (async () => {
      // Global channels (team_id IS NULL) — accessible to everyone
      const { data: globals } = await supabase
        .from("chat_channels")
        .select("*")
        .is("team_id", null)
        .order("created_at");
      const globalRows = (globals ?? []) as Channel[];
      setGlobalChannels(globalRows);
      ...
      // Auto-select: prefer global chat-room
      if (!activeChannel) {
        const room = globalRows.find((c) => c.name === "chat-room") ?? globalRows[0] ?? teamChannels[0];
        ...
        <div className="p-4 border-b border-border">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[10px] text-muted-foreground -mt-0.5">Team & global chat</p>
            </div>
          </div>
        </div>
        ...
        {active ? (
          <>
            <header className="h-14 border-b border-border flex items-center px-4 gap-2">
              ...
              <span className="text-xs text-muted-foreground">
                · {isGlobal ? "Open to everyone" : activeTeam?.name}
              </span>
            </header>
          ...
          <div className="flex-1 flex items-center justify-center text-center p-8">
            <div>
              ...
              <p className="mt-3 text-sm text-muted-foreground">Loading channels…</p>
            </div>
          </div>
        )}
      </main>
